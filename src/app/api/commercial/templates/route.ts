...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${MARVEO_OS_BASE_URL.replace(/\/$/, '')}/api/public/templates${query ? `?${query}` : ''}`, {
      method: 'GET',
      cache: 'no-store',
    ...
    if (!response.ok || !payload) {
      return NextResponse.json({ error: payload?.error || 'Failed to load templates.' }, { status: response.status || 502 });
    }
