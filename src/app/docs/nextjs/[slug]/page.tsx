...
import { getNextJsArticle, NEXTJS_ARTICLES } from "../articles";
...
export function generateStaticParams() {
  return NEXTJS_ARTICLES.map((article) => ({ slug: article.slug }));
}

export default async function NextJsArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const article = getNextJsArticle(slug);
  ...
  const relatedArticles = article.relatedSlugs
    .map((relatedSlug) => getNextJsArticle(relatedSlug))
        ...
        <section className="max-w-4xl mx-auto px-6 pb-24">
          <div className="card-surface rounded-3xl p-6 md:p-8 mb-8">
            <div className="flex flex-wrap items-center gap-3 text-sm text-text-muted mb-5">
              <Link href="/docs/nextjs" className="inline-flex items-center gap-1.5 hover:text-text-primary transition-colors">
                <ArrowLeft className="w-4 h-4" />
                Back to Next.js Help
              </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {relatedArticles.map((relatedArticle) => (
              <article key={relatedArticle.slug} className="card-surface rounded-2xl p-6">
                ...
                <Link
                  href={`/docs/nextjs/${relatedArticle.slug}`}
