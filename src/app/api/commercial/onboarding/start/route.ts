...
export async function POST(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${MARVEO_OS_BASE_URL.replace(/\/$/, '')}/api/public/onboarding/start`, {
      method: 'POST',
      cache: 'no-store',
    ...
    if (!response.ok || !payload) {
      return NextResponse.json({ error: payload?.error || 'Failed to start onboarding.' }, { status: response.status || 502 });
    }
