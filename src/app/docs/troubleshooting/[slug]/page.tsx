...
import { getTroubleshootingArticle, TROUBLESHOOTING_ARTICLES } from "../articles";
...
export function generateStaticParams() {
  return TROUBLESHOOTING_ARTICLES.map((article) => ({ slug: article.slug }));
}

export default async function TroubleshootingArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const article = getTroubleshootingArticle(slug);
        ...
        <section className="max-w-4xl mx-auto px-6 pb-24">
          <div className="card-surface rounded-3xl p-6 md:p-8 mb-8">
            <div className="flex flex-wrap items-center gap-3 text-sm text-text-muted mb-5">
              <Link href="/docs/troubleshooting" className="inline-flex items-center gap-1.5 hover:text-text-primary transition-colors">
