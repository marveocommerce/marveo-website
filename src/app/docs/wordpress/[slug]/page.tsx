...
import { getWordPressArticle, WORDPRESS_ARTICLES } from "../articles";
...
export function generateStaticParams() {
  return WORDPRESS_ARTICLES.map((article) => ({ slug: article.slug }));
}

export default async function WordPressArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const article = getWordPressArticle(slug);
  ...
  const relatedArticles = article.relatedSlugs
    .map((relatedSlug) => getWordPressArticle(relatedSlug))
        ...
        <section className="max-w-4xl mx-auto px-6 pb-24">
          <div className="card-surface rounded-3xl p-6 md:p-8 mb-8">
            <div className="flex flex-wrap items-center gap-3 text-sm text-text-muted mb-5">
              <Link href="/docs/wordpress" className="inline-flex items-center gap-1.5 hover:text-text-primary transition-colors">
                <ArrowLeft className="w-4 h-4" />
                Back to WordPress Help
              </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {relatedArticles.map((relatedArticle) => (
              <article key={relatedArticle.slug} className="card-surface rounded-2xl p-6">
                <p className="text-helper font-mono uppercase tracking-widest text-text-muted mb-2">Related article</p>
                <h3 className="font-display text-lg font-700 text-text-primary mb-2">{relatedArticle.title}</h3>
                <p className="text-sm text-text-secondary leading-relaxed mb-4">{relatedArticle.description}</p>
                <Link
                  href={`/docs/wordpress/${relatedArticle.slug}`}
